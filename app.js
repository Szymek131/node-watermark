...
const prepareOutputFilename = fileName => {
  const splittedText = fileName.split('.')
  const preparedFileName = splittedText[0] + '-with-watermark.' + splittedText[1];
...
const startApp = async () => {
  ...
  if (options.watermarkType === 'Text watermark') {
    ...
    if ((fs.existsSync('./img/' + options.inputImage)) && (fs.existsSync('./img/' + options.watermarkImage))) {
      ...
      console.log('input image or watermark image does not exist !');
      startApp();
    }
  }
};
